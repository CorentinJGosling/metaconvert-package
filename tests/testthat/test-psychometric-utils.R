# =============================================================================
# Psychometric utilities (reliability_change_score, compute_sem, compute_sdc)
# — promoted from tests_save/checked/ (P26, audit digest [31]).
#
# Adapted to the phase-1 fixes in R/psychometric_utils.R:
#   - P21: a negative change-score reliability now warns (value preserved).
#   - P19: the same-sample (no icc_se) CI is now the exact chi-square
#     interval; the sem_se itself is unchanged, so the SE expectations below
#     still hold, and a chi-square CI expectation is asserted where the
#     archived file only checked bracketing.
# =============================================================================
...
test_that("reliability_change_score: rel < r_pre_post yields negative value WITH a warning (P21)", {
  # When rel < r_pre_post, the change-score reliability is negative —
  # population-impossible under classical test theory, so the phase-1 fix
  # added a warning; the value itself is preserved (faithful transcription).
  expect_warning(
    res <- reliability_change_score(reliability = 0.50, r_pre_post = 0.70),
    "Negative change-score reliability"
  )
...
test_that("compute_sem same-sample CI is the exact chi-square interval (P19)", {
  # In the same-sample regime SEM = sqrt(MSE) with MSE/sigma_e^2 ~ chi2_df/df,
  # df = (n-1)(k-1), so the exact interval is
  # [SEM*sqrt(df/qchisq(.975,df)), SEM*sqrt(df/qchisq(.025,df))].
  sd_val <- 10; icc_val <- 0.85; n <- 100; k <- 2
  res <- compute_sem(sd = sd_val, icc = icc_val, n_sample = n, n_measurements = k)
  sem <- sd_val * sqrt(1 - icc_val)
  df <- (n - 1) * (k - 1)
