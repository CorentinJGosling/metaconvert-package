...
# -----------------------------------------------------------------------------
# Test 8.2: morris_dz matches a LIVE metafor escalc(SMCC) call from the t-stat
# -----------------------------------------------------------------------------
test_that("PAIRED-T-SG: morris_dz matches metafor escalc(SMCC, ti=) (g + g_se)", {
  skip_if_not_installed("metafor")
  # Prepare data (compute paired t from pre/post data)
  res <- data.frame(
    study_id = df_base$study_id,
  ...
  # Live external reference: escalc accepts the paired t directly and returns
  # the bias-corrected g_z with vi = 1/n + g^2/(2n). This is an executed
  # metafor call, not a re-typed formula: it also guards the SE convention
  # (an earlier version of the paired-t route used J^2/n + g^2/(2n), which
  # this comparison rejects).
  ref <- metafor::escalc(
    measure = "SMCC",
    ti = res$paired_t_exp, ni = res$n_exp, ri = res$r_pre_post_exp
  )

  expect_equal(es_mc$es_crude, as.numeric(ref$yi), tolerance = 1e-8)
  expect_equal(es_mc$se_crude, sqrt(as.numeric(ref$vi)), tolerance = 1e-8)
})
