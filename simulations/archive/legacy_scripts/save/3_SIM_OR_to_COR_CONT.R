...
r=0.5; n=200; p1=0.3; p2=0.5
or_to_z <- function(r, n, p1, p2) {

  res <- as.data.frame(
    MASS::mvrnorm(n=n, mu=c(0, 0),
                  Sigma=rbind(c(1, r), c(r, 1))))

  res_exp = res[1:(nrow(res)*p1), ]
  res_nexp = res[1:(nrow(res)*(1-p1)), ]
  ...
  n_cases_exp = nrow(subset(res, V1 >= cut2))
  n_controls_exp = nrow(subset(res, V1 < cut2))
  n_cases_nexp = nrow(subset(res, V2 >= cut1))
  n_controls_nexp = nrow(subset(res, V2 < cut1))
  r_raw_data2 = psych::phi(matrix(c(n_cases_exp, n_cases_nexp,
                                    n_controls_exp, n_controls_nexp), nrow=2))
  c(r_raw_data, r_raw_data2)
