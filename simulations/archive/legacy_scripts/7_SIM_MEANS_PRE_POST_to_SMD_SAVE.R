...
means_pre_post_to_smd <- function(d_posttest, d_pre,
  ...
  d_bonett_exp <- (mean_pre_exp - mean_exp) / mean_pre_sd_exp
  g_exp_1 <- d_bonett_exp * J_exp

  d_var_bonett_exp <- var_exp_1 / (mean_pre_sd_exp^2 * (n_exp - 1)) + g_exp_1^2 / (2 * (n_exp - 1))

  d_bonett_nexp <- (mean_pre_nexp - mean_nexp) / mean_pre_sd_nexp
  g_nexp_1 <- d_bonett_nexp * J_nexp

  var_nexp_2 <- mean_pre_sd_nexp^2 + mean_sd_nexp^2 - 2 * r_pre_post_guess * mean_pre_sd_nexp * mean_sd_nexp
  d_var_bonett_nexp <- var_nexp_2 / (mean_pre_sd_nexp^2 * (n_nexp - 1)) + g_nexp_1^2 / (2 * (n_nexp - 1))
  ...
  d_cooper_exp <- (mean_pre_exp - mean_exp) / sd_diff_exp * sqrt(2 * (1 - r_pre_post_guess))

  d_cooper_nexp <- (mean_pre_nexp - mean_nexp) / sd_diff_nexp * sqrt(2 * (1 - r_pre_post_guess))

  d_var_cooper_exp <- 2 * (1 - r_pre_post_guess)/n_exp + d_cooper_exp^2 / (2*n_exp)
  d_var_cooper_nexp <- 2 * (1 - r_pre_post_guess)/n_nexp + d_cooper_nexp^2 / (2*n_nexp)
  ...
  for (met in c("bonett", "cooper", "z", "av")) {
    assign(paste0("d_", met), mget(paste0("d_", met, "_exp"))[[1]] - mget(paste0("d_", met, "_nexp"))[[1]])
    assign(paste0("d_se_", met),
           sqrt(mget(paste0("d_var_", met, "_exp"))[[1]]+mget(paste0("d_var_", met, "_nexp"))[[1]]))
    assign(paste0("d_ci_lo_", met), mget(paste0("d_", met))[[1]] -
             mget(paste0("d_se_", met))[[1]] * qt(0.975, df=n_exp+n_nexp-2))
    assign(paste0("d_ci_up_", met), mget(paste0("d_", met))[[1]] +
             mget(paste0("d_se_", met))[[1]] * qt(0.975, df=n_exp+n_nexp-2))
  }
...
dat = rbind(
  read.delim("D:/simulations/data/MEANS_PRE_POST_to_SMD_sim1000a.txt"))
...
checks = dat %>%
  group_by(d_posttest, r_pre_post) %>%
...
res = dat %>%
  group_by(d_posttest, d_time_common, d_pre, n, r_pre_post, p, r_pre_post_guess) %>%
  summarise(
    n_sim = n(),
    ...
    bias_es_d_av = mean(bias_es_d_av),
    var_d_av = var(d_av),
    mean_d_var_av = mean(d_var_av),
    bias_ci_d_av = mean(ci_cov_d_av),
...
err_plot = res %>%
  ...
  pivot_longer(cols = starts_with("bias_es") |
               ...
               values_to="bias") %>%
  group_by(d_posttest, d_pre, d_time_common, n, r_pre_post, p, r_pre_post_guess, method_long) %>%
  ...
  mutate(pivot = case_when(grepl("bias_es", method_long, fixed = TRUE) ~ "_es",
         ...
         method = case_when(grepl("bonett", method_long, fixed = TRUE) ~ "bonett",
                            grepl("av", method_long, fixed = TRUE) ~ "av",
...
err_plot$d_time_common <- paste0("Pre-post evolution (d=", err_plot$d_time_common, ")")
...
err_plot$method[err_plot$method == "av"] <- "Cumming (Cohen's av)"
