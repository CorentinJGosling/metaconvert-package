...
cont_to_z <- function(r, n, p1, p2) {
  ...
  z_ci_lo_tetrachoric <- z_tetrachoric - qnorm(.975) * z_se_tetrachoric
  z_ci_up_tetrachoric <- z_tetrachoric + qnorm(.975) * z_se_tetrachoric
  r_ci_lo_tetrachoric <- r_tetrachoric - qnorm(.975) * r_se_tetrachoric
  r_ci_up_tetrachoric <- r_tetrachoric + qnorm(.975) * r_se_tetrachoric
...
start_time <- Sys.time()
MC_result <- MonteCarlo(func = cont_to_z,
                        time_n_test = TRUE,
...
rio::export(data_sim, "D:/simulations/data/2x2_to_COR_CONT_sim2500b.txt")

res_cont = read.delim("D:/simulations/data/2x2_to_COR_CONT_sim2500a.txt")

res_cont[res_cont==Inf|res_cont==-Inf|res_cont==9e9] <- NA
checks = res_cont %>%
  group_by(r) %>%
...
rio::export(err_plot %>%
            ...
            "C:/Users/Corentin Gosling/drive_gmail/Recherche/metaConvert/simulations/data_agg/2x2_to_COR_CONT_AGG2500a.txt")
rio::export(err_plot %>%
              filter(method %in% c("Cooper (r)", "Cooper (z)",
                                   "Digby (r)", "Digby (z)",
                                   "Pearson (r)", "Pearson (z)",
                                   "Bonett (r)", "Bonett (z)")),
            "C:/Users/Corentin Gosling/drive_gmail/Recherche/metaConvert/simulations/data_agg/OR_to_COR_CONT_AGG2500a.txt")
...
res = dat %>%
  ...
  summarise(
    ...
    bias_es_z_lipsey = mean_na(bias_es_z_lipsey),
    var_z_lipsey = var(r_lipsey),
    mean_z_var_lipsey = mean_na(r_var_lipsey),
    bias_ci_z_lipsey = mean_na(ci_cov_z_lipsey),

    bias_es_z_tetrachoric = mean_na(bias_es_z_tetrachoric),
    var_z_tetrachoric = var(r_tetrachoric),
    mean_z_var_tetrachoric = mean_na(r_var_tetrachoric),
    bias_ci_z_tetrachoric = mean_na(ci_cov_z_tetrachoric),
...
err_plot$n <- factor(err_plot$n, levels = c("25", "50", "75", "100", "300"),
                     labels = c("n=25", "n=50", "n=75", "n=100", "n=300"))
rio::export(err_plot, "C:/Users/Corentin Gosling/drive_gmail/Recherche/metaConvert/simulations/data_agg/2x2_to_COR_CONT_AGG2500a.txt")
