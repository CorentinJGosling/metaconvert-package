...
## calls NEITHER kernel -- it reimplements the arithmetic inline, so there are four
## pre/post code paths, not three.
##
## These tests install call counters on the two kernels and assert which ones each
## route actually reaches. That turns the README table into something executable,
## so the next person to change the dispatch breaks a test instead of silently
...
## test-INTERNAL-FULL-LIFECYCLE.R for the inline paired path).
## =============================================================================
...
test_that("the paired-t/F family is a FOURTH path -- it calls neither kernel", {
  .needs_pkg()
  ## This is what a three-row table could not represent. These five routes
  ## reimplement the single-group arithmetic in place (R/es_from_PAIRED_STATISTICS.R,
  ## R/es_from_PAIRED_SINGLE_GROUP.R), so a change to either kernel leaves them
  ## untouched and no simulation notices.
  ...
  for (f in names(calls)) {
    n <- .count_kernel_calls(do.call(f, calls[[f]]))
    expect_equal(unname(n[["single_group"]]), 0L, info = f)
    expect_equal(unname(n[["pooled"]]), 0L, info = f)
  }
})
