...
test_that("raw_means + SD => Z (LIPSEY-COOPER)", {
  ...
  # The Fisher-z CI is the delta-method transform of metaConvert's own lipsey_cooper r
  # output (r and var.r are externally validated against esc / compute.es in the "=> R"
  # test above): z = atanh(r), z_se = r_se / (1 - r^2). esc's ci.lo.z/ci.hi.z apply the
  # r-scale SE directly on the z scale WITHOUT the (1 - r^2) Jacobian and are therefore
  # NOT Fisher-consistent -- they diverge from the correct bounds for large |d| (they
  # agree as d -> 0). metaConvert intentionally uses the Fisher-consistent transform
  # (see test-lipsey-cooper-z-var.R), so anchor against it, not esc's bounds.
  es.mcv_r <- summary(convert_df(dat,
    verbose = FALSE, es_selected = "hierarchy", hierarchy = "means_sd", measure = "r",
    smd_to_cor = "lipsey_cooper"
  ), digits = 11)
  z_hw <- qnorm(.975) * es.mcv_r$se_crude / (1 - es.mcv_r$es_crude^2)
  expect_equal(es.mcv_z$es_ci_lo_crude, atanh(es.mcv_r$es_crude) - z_hw, tolerance = 1e-8)
  expect_equal(es.mcv_z$es_ci_up_crude, atanh(es.mcv_r$es_crude) + z_hw, tolerance = 1e-8)
})
