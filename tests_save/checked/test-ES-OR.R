...
test_that("OR+SE to Z (lipsey_cooper 2)", {
  ...
  # esc::convert_d2r()'s z-SE (esc_d2r$se) is NOT the Fisher transform of its own r
  # variance; metaConvert intentionally uses the Fisher-consistent delta value
  # se(z) = se(r)/(1 - r^2). The z POINT estimate still matches esc (atanh of esc's r).
  es.mcv_rr <- summary(convert_df(dat, verbose = FALSE, es_selected = "hierarchy",
    hierarchy = "or_se", measure = "r", or_to_cor = "lipsey_cooper"), digits = 11)
  expect_equal(es.mcv_r$es_crude, atanh(esc_d2r$es), tolerance = 1e-10)
  expect_equal(es.mcv_r$se_crude,
               es.mcv_rr$se_crude / (1 - es.mcv_rr$es_crude^2),
               tolerance = 1e-9)
})
