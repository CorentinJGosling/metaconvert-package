...
#'   \item \code{se_outlier_missing_n} (default FALSE) and \code{se_missing_n_ratio} (default 5): opt-in check (D2c) for a row that reports a standard error but no sample size. The sample-size-normalised SE-outlier checks (D2/D2b) drop such a row silently, so nothing distinguishes "checked and found fine" from "never checked", and no threshold reaches it. When enabled, a dropped row whose \emph{raw} standard error is more than \code{se_missing_n_ratio}-fold tighter than the pool median is flagged. It is confined to rows the normalised checks could not see, so it can add a flag but never change one they raised. Off by default because it is the only check comparing un-normalised standard errors
