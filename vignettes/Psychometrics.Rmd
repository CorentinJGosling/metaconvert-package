...
The SE of the **transformed** ICC depends on the ICC model type. By the delta method, the $(1-\rho)^2$ term from the raw ICC variance cancels with the $1/(1-\rho)^2$ from the transformation derivative --- this cancellation is the variance-stabilising property of the Bonett transformation:

- **ICC(2,1) -- absolute agreement** (`icc_type = "agreement"`):
$$\text{SE}(T) = \sqrt{\frac{2(1+(k-1)\rho)^2}{k(k-1)(n-1)}}$$

- **ICC(3,1) -- consistency** (`icc_type = "consistency"`):
$$\text{SE}(T) = \sqrt{\frac{2}{(k-1)(n-1)}}$$

where $k$ is the number of measurement occasions and $\rho$ is the ICC value. Note that for ICC(3,1), the SE is fully independent of $\rho$.
...
Note that studies using different ICC models (agreement vs. consistency) can coexist in the same analysis. The SE is computed correctly for each study based on its `icc_type`. When pooling, `icc_type` can be examined as a **moderator** variable.
